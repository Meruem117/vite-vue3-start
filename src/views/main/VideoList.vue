<template>
  ...
  <div class="flex flex-col flex-auto flex-nowrap w-full h-full pt-24 p-8 bg-gray-100">
    <!-- up -->
    <div class="flex h-1/4 w-4/5 mx-auto mb-5 rounded bg-no-repeat bg-cover"
      ...
      <div class="flex justify-start px-5 pt-32 pb-5 w-full">
        ...
        <div class="flex flex-col ml-5 mt-2 h-12 justify-start tracking-wider">
          <a class="text-white hover:text-white" :href="`https://space.bilibili.com/${state.mid}`" target="_blank">
            <p class="font-bold text-2xl">{{ state.up.card.name }}</p>
          </a>
          <p class="-mt-5 font-light text-sm font-mono text-white" :title="state.up.card.sign">{{ state.up.card.sign }}
          </p>
        </div>
      </div>
      <div class="flex text-right w-1/6 p-1 text-base justify-center font-mono" :title="state.up.card.fans + '个粉丝'">粉丝数:
        {{ convertNumber(state.up.card.fans) }}</div>
    </div>
    <!-- video -->
    <div class="w-4/5 h-3/4 bg-white p-5 rounded mx-auto">
      <div class="grid grid-cols-5 gap-5">
        <div v-for="video in state.videos[state.current - 1]" class="flex flex-col flex-nowrap flex-auto h-64">
          <!-- 封面 -->
          <div :style="{ backgroundImage: `url('${convertImageUrl(video.pic)}')` }"
            class="flex rounded cursor-pointer bg-no-repeat bg-cover h-full w-full" @click="toVideo(video.bvid)">
          ...
          <!-- 信息 -->
          <p class="h-20 text-base overflow-hidden mt-2 hover:text-blue-400 cursor-pointer video-title"
            :title="video.title" @click="toVideo(video.bvid)">{{ video.title }}</p>
      ...
      <div class="flex justify-center mt-6" @click="handleChangePage">
        <a-pagination v-model:current="state.current"
